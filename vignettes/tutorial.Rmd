---
title: "`EloRating` - a brief tutorial \\ version 0.45.03"
...
```{r}
library(EloRating2)
xdata <- read.table(system.file("ex-sequence.txt", package = 'EloRating2'), header = TRUE)
```
\normalsize
...
\small
```{r}
xpres <- read.table(system.file("ex-presence.txt", package = "EloRating2"), header = TRUE)
...
xres <- elo.seq(winner = rdata$seqdat$winner, loser = rdata$seqdat$loser, Date = rdata$seqdat$Date, 
                presence = rdata$pres, progressbar = F)
summary(xres)
```
\normalsize
