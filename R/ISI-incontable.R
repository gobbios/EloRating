...
.incontable <- function(mat) {
  ...
  if(.incon(mat) > 0) {
    ...
    for (i in 1:(N-1)) {
      ...
      res <- cbind(res, rbind(names(which(ROW-COL < 0)), rep(colnames(mat)[i], length(names(which(ROW-COL < 0))))))
    }
    ...
    res <- as.data.frame(t(rbind(res, SIs))); colnames(res)[1:2] <- c("too low", "too high")
